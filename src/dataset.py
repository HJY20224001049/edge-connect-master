...
from.utils import create_mask, imsave
import cv2
...
class Dataset(torch.utils.data.Dataset):
    ...
    def load_edge(self, img, index, mask):
        ...
        if self.edge == 1:
            ...
            edge = canny(img, sigma=sigma, mask=mask).astype(float)
            # 保存边缘图到本地
            imsave(edge, self.get_save_path(index))
    ...
    def get_save_path(self, index):
        ...
        save_path = os.path.join(base_dir, name + "_edge" + ext)
        return save_path